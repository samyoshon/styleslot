class UsersController < ApplicationController
	...
	def index
        ...
        @users = @q.result.paginate(page: params[:page], per_page: 30).where(["updated_at > ?", 1.days.ago]).order("updated_at DESC")

        ## Find Countries That Have Job Posts - Used for filter option
        @resumes = User.where(["updated_at > ?", 1.days.ago]).order("created_at DESC")
